...
#pragma region 描画初期化処理
    ...
    HRESULT result = directXCommon->GetDevice()->CreateCommittedResource(
        &heapProp, // ヒープ設定
        D3D12_HEAP_FLAG_NONE,
        &resDesc, // リソース設定
        D3D12_RESOURCE_STATE_GENERIC_READ,
    ...
    assert(SUCCEEDED(result));
    result = directXCommon->GetDevice()->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature));
    assert(SUCCEEDED(result));
    rootSigBlob->Release();
    // パイプラインにルートシグネチャをセット
...
#pragma endregion
    ...
    for (int i = 0; i < _countof(object3ds); i++) {
        // 初期化
        InitializeObject3d(&object3ds[i], directXCommon->GetDevice());
    ...
    ComPtr<ID3D12Resource> texBuff1 = CreateTexture(directXCommon->GetDevice(), metadata1);
    ...
    ComPtr<ID3D12Resource> texBuff2 = CreateTexture(directXCommon->GetDevice(), metadata2);
    ...
    directXCommon->GetDevice()->CreateShaderResourceView(texBuff1.Get(), &srvDesc, srvHandle);

    // 2枚目のシェーダーリソースビューを作成。1枚目のディスクリプタハンドルの次の場所に配置する
    size_t incrementSize = directXCommon->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    srvHandle.ptr += incrementSize;
    srvDesc.Texture2D.MipLevels = UINT(metadata2.mipLevels);
    directXCommon->GetDevice()->CreateShaderResourceView(texBuff2.Get(), &srvDesc, srvHandle);
    ...
    while (true) {
        ...
        directXCommon->GetCommandList()->SetDescriptorHeaps(1, descHeaps);
        // SRVヒープの先頭ハンドルを取得（SRVを指しているはず）
        D3D12_GPU_DESCRIPTOR_HANDLE srvGpuHandle = srvHeap->GetGPUDescriptorHandleForHeapStart();
        srvGpuHandle.ptr += textureIndex * directXCommon->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        ...
        for (int i = 0; i < _countof(object3ds); i++)
        {
            DrawObject3d(&object3ds[i], directXCommon->GetCommandList(), vbView, ibView, _countof(indices));
        }
